...
def main():
    ...
    LIGHTINFO = "/exp/icarus/data/users/mvicenzi/pmt-timing/flashana/{}/flashana_files_run{}.txt".format(RUN,RUN) 
    OUTFILE = "output/residuals/Run_2/run{}_residuals_testStandard_flashana_CORRw{}.csv".format(RUN,CORR)
    ...
    dfw, dfe = loadFiles(FILENAMES, "flashana/opflashCryoW_flashtree", "flashana/opflashCryoE_flashtree", maxim)
    dfw["cryo"] = 1
    dfe["cryo"] = 0
...
#    plt.savefig("figs/run{}_tracks_standard_selection.png".format(RUN),dpi=300)
    ...
    COSMICSCORR = "output/residuals/Run_2/run{}_residuals_testStandard_flashana.csv".format(CORR)
    ...
    #add off channels
    geo = loadSingleFile(FILENAMES[0], "flashana/geotree")
